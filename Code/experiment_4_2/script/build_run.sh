...
WRAP3_SYMS=( dsytrd_ dorgtr_ dsteqr_ dstevd_ )
WRAP3_LDFLAGS=()
...
case "$TAG" in
  ...
  dsyev-lapack)
      SRC="$SCRIPT_DIR/../src/dsyev.c"
      CFLAGS="$CFLAGS_NETLIB"
      LDFLAGS="$LDFLAGS_NETLIB"
      ;;

  dsyevd-lapack)
      SRC="$SCRIPT_DIR/../src/dsyevd.c"
      CFLAGS="$CFLAGS_NETLIB"
      LDFLAGS="$LDFLAGS_NETLIB"
      ;;

  dsyev_dsyevd_compare-openblas)
      SRC="$SCRIPT_DIR/../src/dsyev_dsyevd_compare.c"
      CFLAGS="$CFLAGS_OB"
      LDFLAGS="$LDFLAGS_OB"
      ;;
