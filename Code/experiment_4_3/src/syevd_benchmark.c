...
int main(void) {
    ...
    if (csv) fprintf(csv, "lib,routine,threads,N,total_s,dsytrd_s,dstedc_s,dormtr_s\n");
    ...
    for (int ti = 0; ti < nthr; ++ti) {
        ...
        for (int si = 0; si < nsizes; ++si) {
            ...
            /* Reset subroutine timers for this (t,n) measurement */
            timers_reset();
            ...
            /* Print row: calls are what wrappers observed */
            printf("%-8d | %-6s | %-10.3f | %-10.3f | %-10.3f | %-10.3f\n",
                   n, "-", total_s, G_TIMERS.dsytrd_s, G_TIMERS.dstedc_s, G_TIMERS.dormtr_s);

            /* CSV */
            if (csv) fprintf(csv, "%s,%s,%d,%d,%.6f,%.6f,%.6f,%.6f\n",
                             lib_tag, routine, t, n, total_s,
                             G_TIMERS.dsytrd_s, G_TIMERS.dstedc_s, G_TIMERS.dormtr_s);
            ...
            /* -------- Lightweight file output: first 5 eigenvalues and first 5 full columns -------- */
            {
                ...
                /* Write first up to OUTPUT_K eigenvalues (ascending order) */
