#!/usr/bin/env bash
# Run full experiment 4.3: benchmarks (SVE + SIMD baseline) then compare + stack_timing.
...
python3 "$SCRIPT_DIR/compare_partial.py" --lib-a openblas_sve --lib-b openblas_simd --routine syev  --root "$ROOT"
python3 "$SCRIPT_DIR/compare_partial.py" --lib-a openblas_sve --lib-b openblas_simd --routine syevd --root "$ROOT"
