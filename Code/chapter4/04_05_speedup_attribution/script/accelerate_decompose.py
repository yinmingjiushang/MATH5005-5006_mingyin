...
def main():
    ...
    default_root_42 = os.path.abspath(os.path.join(script_dir, "..", "..", "04_03_blas_optimization", "output"))
    default_root_43 = os.path.abspath(os.path.join(script_dir, "..", "..", "04_04_sve_vectorization", "output"))

    ap = argparse.ArgumentParser(description="Acceleration decomposition (4.4)")
    ap.add_argument("--root4_2", default=default_root_42, help="4.3 BLAS-comparison output root")
    ap.add_argument("--root4_3", default=default_root_43, help="4.4 SIMD-comparison output root")
    ...
    # 4.3/4.4 legacy naming: older outputs may use "scalar" where the chapter text now says "simd"
    syev_43 = first_existing([
        os.path.join(compare_43, "time_syev_stacked_openblas_sve_openblas_simd.csv"),
        os.path.join(compare_43, "time_syev_stacked_openblas_sve_openblas_scalar.csv"),
    ])
    syevd_43 = first_existing([
        os.path.join(compare_43, "time_syevd_stacked_openblas_sve_openblas_simd.csv"),
        os.path.join(compare_43, "time_syevd_stacked_openblas_sve_openblas_scalar.csv"),
    ])
    ...
    def get_43(routine, lib, t, n, col):
        ...
        if not os.path.isfile(path):
            return None
        aliases = {
            "openblas_simd": {"openblas_simd", "openblas_scalar"},
            "openblas_scalar": {"openblas_scalar", "openblas_simd"},
            "openblas_sve": {"openblas_sve"},
        }
        accepted_libs = aliases.get(lib, {lib})
        for row in load_simple_csv(path):
            if int(row["threads"]) == t and int(row["N"]) == n and row["lib"] in accepted_libs:
                return float(row.get(col, 0))
        return None
    ...
    out_path = args.out
    if out_path is None:
        out_dir = os.path.abspath(os.path.join(script_dir, "..", "output", "compare"))
