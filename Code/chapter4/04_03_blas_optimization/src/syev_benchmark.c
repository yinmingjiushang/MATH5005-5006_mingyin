...
// Current default configuration is the controlled single-thread subset used
// for the stage-by-stage library-stack comparison in Chapter 4.
//
...
int main(void) {
    ...
    if (csv) {
        fprintf(csv,
                "lib,routine,threads,N,reported_stat,repeats,warmup_runs,"
                "total_s,dsytrd_s,dorgtr_s,dsteqr_s,"
                "total_mean_s,total_std_s,total_min_s,total_max_s,"
                "dsytrd_mean_s,dsytrd_std_s,dorgtr_mean_s,dorgtr_std_s,dsteqr_mean_s,dsteqr_std_s\n");
    }
    ...
    for (int ti = 0; ti < nthr; ++ti) {
        ...
        for (int si = 0; si < nsizes; ++si) {
            ...
            double dorgtr_runs[MAX_MEASURE_RUNS];
            double dsteqr_runs[MAX_MEASURE_RUNS];
            ...
            for (int warm = 0; warm < WARMUP_RUNS; ++warm) {
                ...
                if (info) { fprintf(stderr, "DSYEV warm-up failed N=%d info=%d\n", n, info); exit(4); }
            }
            ...
            for (int rep = 0; rep < reps; ++rep) {
                ...
                dorgtr_runs[rep] = G_TIMERS.dorgtr_s;
                dsteqr_runs[rep] = G_TIMERS.dsteqr_s;
            }

            {
                double total_s = stats_median(total_runs, reps);
                double dsytrd_s = stats_median(dsytrd_runs, reps);
                double dorgtr_s = stats_median(dorgtr_runs, reps);
                double dsteqr_s = stats_median(dsteqr_runs, reps);

                printf("%-8d | med[%d] | %-10.3f | %-10.3f | %-10.3f | %-10.3f\n",
                       n, reps, total_s, dsytrd_s, dorgtr_s, dsteqr_s);
                ...
                if (csv) {
                    fprintf(csv,
                            ...
                            "%.6f,%.6f,%.6f,%.6f,"
                            "%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
                            lib_tag, routine, t, n, reps, WARMUP_RUNS,
                            total_s, dsytrd_s, dorgtr_s, dsteqr_s,
                            ...
                            stats_mean(dorgtr_runs, reps), stats_stddev(dorgtr_runs, reps),
                            stats_mean(dsteqr_runs, reps), stats_stddev(dsteqr_runs, reps));
                }
            }
