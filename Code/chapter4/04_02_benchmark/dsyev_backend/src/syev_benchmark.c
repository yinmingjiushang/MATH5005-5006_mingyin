...
int main(void) {
    ...
    ensure_dir("../output");
    FILE *csv = fopen("../output/syev_benchmark.csv", "w");
    if (csv)
        fprintf(csv,
    ...
    for (int ti = 0; ti < nthr; ++ti) {
        ...
        for (int si = 0; si < nsizes; ++si) {
            ...
            {
                char dir[128];
                snprintf(dir, sizeof(dir), "../output/run_syev_T%d_N%d", t, n);
                ensure_dir(dir);
                ...
                /* Write first up to 5 eigenvalues */
                ...
                /* Write first up to 5 eigenvector columns (full length) */
    ...
    if (csv) fclose(csv);
    printf("Saved CSV: ../output/syev_benchmark.csv\n");
    return 0;
}
