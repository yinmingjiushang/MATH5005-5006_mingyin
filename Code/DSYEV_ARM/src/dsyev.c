...
// NOTE: DSYEV has NO IWORK/LIWORK.
...
int main(void) {
    ...
    if (!WORK) { fprintf(stderr, "Workspace allocation failed.\n"); return 3; }

    // 2) Actual computation (timed)
    ...
    printf("DSYEV computation took %.3f seconds.\n", elapsed_sec);

    // Outputs (outside timing)
    ...
    if (ft) { fprintf(ft, "DSYEV computation took %.3f seconds.\n", elapsed_sec); fclose(ft); }
